...
|`--build true`
|Builds the application after generation. If you want to build it manually, you can omit this flag and run `mvn clean install` in the generated directory.

|`--name`
|Specifies the name of the generated service. The value is used as the base for the project name, artifact name, folder name, Helm metadata and application metadata.

|`--group`
|Specifies the `groupId` of the generated service. The value is used for the Maven project configuration, including the parent `groupId`.

|`--package`
|Specifies the base package name of the generated service. The value is used as the base package for generated application parts created from templates, such as controllers, DAOs, entities, mappers, services and exception mappers.
|===
